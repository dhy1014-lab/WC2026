...
export default function WorldCupPool() {
  ...
  const p1AdminLocked = isPhase2Locked() && !settings.p1Unlock; // P1 edit locked once knockout stage begins
            ...
            {adminTab==="audit" && (() => {
              ...
              return (
                <div>
                  ...
                  {auditPhase==="p1" && (
                    <div>
                      {/* P1 locked banner — shown after P2 kickoff */}
                      {isPhase2Locked() && (
        ...
        {screen==="leaderboard" && (
          <div>
                            ...
                            return (
                              <table style={{ borderCollapse:"collapse", marginLeft:"auto", fontSize:11 }}>
                                <tbody>
                                  {bPts > 0 && (
                                    <tr>
                                      <td style={{ color:"#9ab8a0", paddingRight:8, textAlign:"right" }}>bracket</td>
                                      <td style={{ color:"#f0e6c8", fontWeight:"bold", textAlign:"right" }}>{bPts}</td>
                                    </tr>
                                  )}
                                  {pPts2 > 0 && (
                                    <tr>
                                      <td style={{ color:"#9ab8a0", paddingRight:8, textAlign:"right" }}>props</td>
                                      <td style={{ color:"#f0e6c8", fontWeight:"bold", textAlign:"right" }}>{pPts2}</td>
                                    </tr>
                                  )}
                                  {gbPts2 > 0 && (
                                    <tr>
                                      <td style={{ color:"#9ab8a0", paddingRight:8, textAlign:"right" }}>boot</td>
                                      <td style={{ color:"#f0e6c8", fontWeight:"bold", textAlign:"right" }}>{gbPts2}</td>
                                    </tr>
                                  )}
                                  <tr style={{ borderTop:"1px solid rgba(255,255,255,0.1)" }}>
                                    <td style={{ color:"#9ab8a0", paddingRight:8, textAlign:"right", paddingTop:3 }}>total</td>
            ...
            {lbTab==="results" && (() => {
              if (lbPhase==="p2") {
                // ── P2 RESULTS ──
                const anyBracketWinner = bracketWinners && Object.keys(bracketWinners).some(k => k !== "finalFirstGoalMinute" && bracketWinners[k]);
                const anyP2Prop = p2PropResults && Object.values(p2PropResults).some(v => v !== null && v !== undefined);
                const gbSettled = !!goldenBoot?.answer;
                if (!isPhase2Open()) {
                  return (
                    <div style={{ ...S.card, textAlign:"center", color:"#aab0ff", fontSize:13 }}>🔜 Phase 2 results will appear once the knockout stage begins (Jun 28).</div>
                  );
                }
                return (
                  <div>
                    ...
                    <div style={{ marginBottom:18 }}>
                      ...
                      {!anyBracketWinner && <div style={{ ...S.card, fontSize:11, color:"#9ab8a0", textAlign:"center" }}>No bracket results settled yet — check back after matches complete.</div>}
                      {Object.entries(ROUND_LABELS).filter(([round]) => KNOCKOUT_ROUNDS[round]?.length).map(([round, roundLabel]) => {
                        const matches = KNOCKOUT_ROUNDS[round] || [];
                        const settled = matches.filter(m => bracketWinners?.[m.id]);
                          ...
                          <div key={round} style={{ marginBottom:10 }}>
                            ...
                            {settled.map(m => {
                              ...
                              const correct = players.filter(pl => {
                                const pick = predictions[pl.id]?.phase2Picks?.[m.id];
                                return pick && resolvePickToTeam(pick, { groupRankings: liveResults?.groupRankings, bracketSlots }) === winner;
                              }).length;
                        ...
                        return (
                          <div key={round} style={{ marginBottom:10 }}>
                            ...
                            {roundProps.map(prop => {
                              ...
                              const yesWon = result === true;
                              const noWon = result === false;
                              const correct = players.filter(pl => {
                                const pick = predictions[pl.id]?.p2PropPicks?.[prop.id];
                                return pick !== null && pick !== undefined && pick === result;
                              }).length;
                              ...
                              return (
                                <div key={prop.id} style={{ background:"rgba(255,255,255,0.04)", borderRadius:6, padding:"8px 10px", marginBottom:5, border:"1px solid rgba(255,255,255,0.09)" }}>
                                  <div style={{ display:"flex", alignItems:"flex-start", gap:8 }}>
                                    <div style={{ fontSize:13, marginTop:1 }}>{yesWon ? "✅" : "❌"}</div>
                                    <div style={{ flex:1 }}>
                                      <div style={{ fontSize:11, color:"#f0e6c8" }}>{prop.q}</div>
                                      <div style={{ display:"flex", gap:10, marginTop:4, flexWrap:"wrap" }}>
                                        <span style={{ fontSize:10, color: yesWon ? "#8fffb0" : "rgba(154,184,160,0.5)", fontWeight: yesWon ? "bold" : "normal" }}>YES {yesWon?"✓":""} · {prop.ptsYes}pts</span>
                                        <span style={{ fontSize:10, color: noWon ? "#8fffb0" : "rgba(154,184,160,0.5)", fontWeight: noWon ? "bold" : "normal" }}>NO {noWon?"✓":""} · {prop.ptsNo}pts</span>
                      ...
                      <div style={{ marginBottom:18 }}>
                        ...
                        <div style={{ background:"rgba(255,255,255,0.04)", borderRadius:6, padding:"10px 12px", border:"1px solid rgba(255,255,255,0.09)" }}>
                          ...
                          {(() => {
                            ...
                            return (
                              <div style={{ fontSize:11, color:"#9ab8a0" }}>
                                {opt ? `Worth ${opt.pts} pts` : ""}{total > 0 ? ` · ${correct}/${total} picked correctly` : ""}
                              </div>
                            );
                          })()}
                        </div>
